/********************************************************************
Test Case Name: Verify 'My account page' when logging with super user credentials 
Test Case ID:http://qa.digitas.com/SpiraTest/523/TestCase/8466.aspx
Objective: Verify 'My account page' when logging with super user credentials
...
public class MyAccount8466 extends BaseClass{
	
	@Test
	public void test_myAccount8466() throws Exception{
		...
		String query = "SELECT client.name as name FROM idiom.client JOIN idiom.user_client " +
				...
				"WHERE user_client.status='ACTIVE' AND client.status='ACTIVE' AND users.email_address='"+adminUser+"'";
		...
		if(myAcct.func_getText("role").contains("Super Admin")){
			CustomReporter.log("User role is Super Admin");
		}else{
			CustomReporter.errorLog("User role is not [super admin] it is= "+ myAcct.func_getText("role"));
			BaseClass.testCaseStatus = false;
		}
		...
		String genereateIdiomName = manageIdidom.func_CreateIdiomName("myAccount8521", "New");
		...
		if(myAcct.func_getText("role").contains("Super Admin")){
			...
			CustomReporter.errorLog("User role is not [super admin] and it is "+myAcct.func_getText("role"));
			BaseClass.testCaseStatus = false;
		}
