/********************************************************************
Test Case Name: *My Account_General  user Login 
Test Case ID: http://qa.digitas.com/SpiraTest/523/TestCase/8471.aspx
Objective: Verify 'My account page' when logging with General user credentials
...
public class MyAccount8471 extends BaseClass{
	
	
	@Test
	public void test_myAccount8471() throws Exception{
		
		/******************************Variables********************************/
		String normalAppUser = property.getProperty("GeneralUserEmail").trim();
		String normalAppPass = property.getProperty("GeneralUserPassword").trim();
		...
		String query = "SELECT client.name as name FROM idiom.client JOIN idiom.user_client " +
				...
				"WHERE user_client.status='ACTIVE' AND client.status='ACTIVE' AND users.email_address='"+normalAppUser+"'";
		...
		lp.func_LoginToIDIOM(normalAppUser, normalAppPass);
		...
		CustomReporter.log("Logged in with normal user");
		...
		if(!rm.webElementSync(myAcct.func_GetLocalWebElement("userRole"), "visibility", "", null)){
			CustomReporter.errorLog("User role  for [Normal App] is not appearing");
			BaseClass.testCaseStatus = false;
		}else{
			CustomReporter.log("User role is :"+myAcct.func_getText("role"));
		}
		...
		String genereateIdiomName = manageIdidom.func_CreateIdiomName("myAccount8471", "New");
		...
		if(!rm.webElementSync(myAcct.func_GetLocalWebElement("userRole"), "visibility")){
			CustomReporter.errorLog("User role  for [Normal App] is not appearing");
			BaseClass.testCaseStatus = false;
		}else{
			CustomReporter.log("User role is :"+myAcct.func_getText("role"));
