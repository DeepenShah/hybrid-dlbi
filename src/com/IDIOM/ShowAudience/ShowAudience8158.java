...
/** 
 * <p> <b>Test Case Name:</b>*Analysis_Show_Audience widget _Verify_Population UI</p>
 * <p> <b>Objective:</b>Verify UI Population  looks same to design.</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8158.aspx/p>
 * <p> <b>Module:</b> Show Audience</p>
 * @author Amrutha Nair
 * @since 16-June-2016
 *
 */
public class ShowAudience8158 extends BaseClass {
		
	@Test
	public void verifyShowAudiencePanel(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8158-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8158-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8158_projectname", "fail");
			}
			...
			//Step 11:Click on '>' button at the bottom bar
			
			AD.arrowLinkClickForSuccessMetricsOrAudienceDefinition();
			strMsg="The user has clicked on > to navigate to profile page";
			CustomReporter.log(strMsg);
			...
			//Step 14:Compare following data in both Projected and Actual Tabs with previously noted data in step 10:
			...
			if(PopulationPercentProjected==SHPopulationPercentProjected){
				strMsg="The projectedpopulation percentage in Show audience panel is matching with the same in success metrics right panel. .The value is:"+SHPopulationPercentProjected;
				CustomReporter.log(strMsg);
			}
			
			else{
				strMsg="The projectedpopulation percentage in Show audience panel is not matching with the same in success metrics right panel. Show audience value is '"+SHPopulationPercentProjected+"' success metrics value is '"+PopulationPercentProjected+"'";
				CustomReporter.errorLog(strMsg);
				status=false;
			}
			
			if(PopulationPercentActual==SHPopulationPercentActual){
				strMsg="The actual population percentage in Show audience panel is matching with the same in success metrics right panel.The value is:"+SHPopulationPercentActual;
				CustomReporter.log(strMsg);
			}
			
			else{
				strMsg="The actual population percentage in Show audience panel is not matching with the same in success metrics right panel. Show audience value is '"+SHPopulationPercentActual+"' success metrics value is '"+PopulationPercentActual+"'";
				CustomReporter.errorLog(strMsg);
				status=false;
			}
			
			if(optActualAdudience==SHoptActualAdudience){
				strMsg="The actual optimal audience  in Show audience panel is matching with the same in success metrics right panel";
				CustomReporter.log(strMsg);
			}
			
			else{
				strMsg="The actual optimal audience  in Show audience panel is not matching with the same in success metrics right panel";
				CustomReporter.errorLog(strMsg);
				status=false;
			}
			
			if(optProjectedAdudience==SHoptProjectedAdudience){
				strMsg="The projected optimal audience  in Show audience panel is matching with the same in success metrics right panel";
				CustomReporter.log(strMsg);
			}
			
			else{
				strMsg="The projected optimal audience  in Show audience panel is not matching with the same in success metrics right panel";
				CustomReporter.errorLog(strMsg);
				status=false;
			}
			boolean flag=true;
			...
			if(SHmetricsActual.size()==metricsActual.size()){
				for(String actual:SHmetricsActual){
					if(!(metricsActual.contains(actual))){
						status=false;
						flag=false;
						strMsg="The actual tab success metrics in Show audience panel are not  matching with the same in success metrics right panel";
						CustomReporter.errorLog(strMsg);
						break;
					}
				}
				if(flag){
					strMsg="The actual tab success metrics in Show audience panel are   matching with the same in success metrics right panel . value is :"+SHmetricsActual;
					CustomReporter.log(strMsg);
				}
			...
			if(SHmetricsProjected.size()==metricsProjected.size()){
				for(String projected:SHmetricsProjected){
					if(!(metricsProjected.contains(projected))){
						status=false;
						flag=false;
						strMsg="The projected tab success metrics in Show audience panel are not  matching with the same in success metrics right panel";
						CustomReporter.errorLog(strMsg);
						break;
					}
				}
				if(flag){
					strMsg="The projected tab success metrics in Show audience panel are   matching with the same in success metrics right panel . value is :"+SHmetricsProjected;
					CustomReporter.log(strMsg);
				}
			}
			else{
			...
			if (status){
				strMsg="The population, optimal audience and success metrics in both projected and actual tabs in Show Audience panel are matching with the data in success metrics right panel";
				CustomReporter.log(strMsg);
			}
			else{
				strMsg="There is data mismatch in show audience panel";
				throw new IDIOMException(strMsg+"###8158_DataMismatch");
				
			}
			
			
			//Step 15:Click again on the "show audience" button
			ap.performAction("showaudience");
			...
			CustomReporter.log(strMsg);
				...
				rm.captureScreenShot("8158", "fail");
			}finally{
				try{
