...
/** 
 * <p> <b>Test Case Name:</b> *1963_AudienceDefinition_VerifQueryStructure </p>
 * <p> <b>Objective:</b> Objective:Verify the query structure in audience definition panel</p>
 * <p> <b>Test Case ID:</b>  http://qa.digitas.com/SpiraTest/523/TestCase/8691.aspx</p>
...
public class AudienceDefinition8691 extends BaseClass {
		
	@Test
	public void verifyQueryStructure(){
		String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8691-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8691-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8691_NoProjectHeading", "fail");
			...
			//Step 6:Click on any of the attribute and Add the same
			...
			strMsg="The quries are added in the edit panel";
			CustomReporter.log(strMsg);
			
			
			//Step 7:Verify the query structure
			if(AD.verifyQueryStructure(2, strAudienceAttributes1))
			{
				strMsg="The query structure is proper , ie [And/OR Parent NAme:Child NAme]";
				CustomReporter.log(strMsg);
				
			}
			else{
				strMsg="The query structure is not proper";
				throw new IDIOMException(strMsg+" .###8691_QueriesStructureNotProper"); 
			}
			...
			rm.captureScreenShot("8691", "fail");
		}finally{
			try{
