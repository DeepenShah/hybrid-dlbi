...
/** 
 * <p> <b>Test Case Name:</b>**Filtering Menu:Bottom SaveBar: 3(e): Verify message on bottom bar if no audience is selected</p>
 * <p> <b>Objective:</b> Verify message "No Attributes Selected" at bottom of success metrics of no audience is selected </p>
 * <p> <b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8613.aspx</p>
 * <p> <b>Module:</b> Audience Builder</p>
 * @author Amrutha Nair
 * @since 11 May 2016
 *
 */
public class AudienceDefinition8613 extends BaseClass {

	@Test
	public void verifyNoAuienceSelectedMsg(){
	String strMsg = null;
		...
		try{
			...
			String strMessage=property.getProperty("noAttributesSelected");
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8613-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8613-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8613-ProjectName", "fail");
			...
			Thread.sleep(2000);
			//Step 8:Navigate to audience definition page
			Thread.sleep(3000);
			...
			//Step 9:	do not select any audience and verify message in right side bottom panel
			if(AD.getNoAttributeSelectedMessage().toLowerCase().contentEquals(strMessage.toLowerCase()))
			{
				strMsg="The proper message is coming in bottom navigation bar when no attributes are selected";
				CustomReporter.log(strMsg);
			
			}
			else{
				strMsg="The proper message is NOT coming in bottom navigation bar when no attributes are selected. The message coming in bottom navigation bar is '"+AD.getNoAttributeSelectedMessage()+"'";
				throw new IDIOMException(strMsg+".###8613-NOAttributesSelectedMessageIssue");
			}
			...
			if(strMsgAndFileName.length==1){
				rm.captureScreenShot("8613-Exception", "fail");
			}else{
				rm.captureScreenShot("8613-"+strMsgAndFileName[1], "fail");	
			}			
