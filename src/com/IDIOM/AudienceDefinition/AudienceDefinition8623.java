...
/** 
 * <p> <b>Test Case Name:</b>*Audience Definition: Query Builder 1.b.ii.1.b - Cancel edit group name</p>
 * <p> <b>Objective:</b> Edit functionality through pencil icon and click cancel button.  </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8623.aspx</p>
 * <p> <b>Module:</b> Audience Builder</p>
 * @author Amrutha Nair
 * @since 04-May-2016
 *
 */
public class AudienceDefinition8623 extends BaseClass {
		
	@Test
	public void verifyEditCancelGroupName(){
		String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8623-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8623-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8623_ProjectNameNotexpected", "fail");
			}
			...
			if(AD.verifyGroupEditableMode("1")){
				...
				CustomReporter.errorLog(strMsg);
				BaseClass.testCaseStatus=false;
				rm.captureScreenShot("8623_GroupNameNotexpected", "fail");
			}
			//Step 8:Edit the name and click 'CANCEL'.
			AD.fillGroupName(groupName+"test");
			
			strMsg="The user has entered new group name";
			CustomReporter.log(strMsg);
			
			AD.performActionOnEditGroupName(false);
			
			strMsg="The user clicked on 'Cancel'";
			CustomReporter.log(strMsg);
			
			if(AD.getGroupName("1").contentEquals(groupName))
			{
				
				strMsg="The group name didn't get updated and prevous name is coming since the user clicked on 'cancel'";
				CustomReporter.log(strMsg);
			}
			else {
				strMsg="The group name got updated eventhough user clicked on 'cancel'";
				throw new IDIOMException(strMsg+" .###8623-GroupIsGettingRenamed");
			}			
			...
			rm.captureScreenShot("8623", "fail");
		}finally{
			try{
