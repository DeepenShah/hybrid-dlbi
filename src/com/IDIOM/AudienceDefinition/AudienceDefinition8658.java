...
/** 
 * <p> <b>Test Case Name:</b>**Audience 2.0 : Client Admin User Journery</p>
 * <p> <b>Objective:</b> Verify user journey for Client Admin User </p>
 * <p> <b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8658.aspx </p>
 * <p> <b>Module:</b> Audience Builder</p>
 * @author Amrutha Nair
 * @since 12 May 2016
 *
 */
public class AudienceDefinition8658 extends BaseClass {

	@Test
	public void verifyClientAdminUserJourney(){
	String strMsg = null;
		...
		try{
			...
			if(AD.verifySubcategoriesForSelectedSuccessMetrics(strSubCategories, expectedCategories)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_SubCategoriesInSuccessMetrics", "fail");
			}
			...
			if(AD.getCountForSuccessMetricsOrAudienceDefinition()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_NumberofslectedCards", "fail");
			}
			...
			if(AD.isVisible("removealllink")){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_RemoveAll", "fail");
			}
			...
			if(AD.getCountForSubCategoriesHavingCorssIcon()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_CrossSignissue", "fail");
			}
			...
			if(AD.getTotalMetricCards()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_SelectedMetricCard", "fail");
			}
			...
			if (strActualVal.contains(strmetricsToBeRemoved.trim())){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_DidNotRemoveSuccessMetric", "fail");
			}
			else{
			...
			if(AD.verifySuccessMetricsSearchfunctionality(strSearchValue)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_SearchFunctionality", "fail");
			}
			...
			if(AD.isVisible("defaultgroup", "")){
				...
				CustomReporter.errorLog(strMsg);
				BaseClass.testCaseStatus=false;
				rm.captureScreenShot("8658NoDefaultGroup", "fail");
			}
			...
			if(AD.getCountForSuccessMetricsOrAudienceDefinition()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_Numberofqueries", "fail");
			}
			...
			if(strQueryLogic.toLowerCase().contentEquals(defaultQueryOrSubGroupLogic.toLowerCase())){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_QueryLogicdefault", "fail");
			...
			if(strQueryLogic.toLowerCase().contentEquals(strLogic)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_QueryLogicChange", "fail");
			}
			...
			if(AD.getQueryWithLogic("1.1").contentEquals(strQuery)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_QueryNotgotdeleted", "fail");
			}
			...
			if(AD.getGroupCount()==(grpCount+1)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_AddGroupissue", "fail");
			}
			...
			if(AD.getSubGroupCount("1")==(intsubgrp+1)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8658_AddsubGroupissue", "fail");
			}
			...
			if(!rm.webElementSync(AP.homeOwnerGraph, "visibility"))
				throw new IDIOMException("Failed to land on profile page.###8658_NotAbleToNavigateToProfilePage");
			...
			if(strMsgAndFileName.length==1){
				rm.captureScreenShot("8658-Exception", "fail");
			}else{
				rm.captureScreenShot("8658-"+strMsgAndFileName[1], "fail");	
			}			
