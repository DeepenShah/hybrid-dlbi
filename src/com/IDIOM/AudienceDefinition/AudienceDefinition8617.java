...
/** 
 * <p> <b>Test Case Name:</b> *Audience Definition - a (Categories).i.2.a - Verify to which demi decile graph should default</p>
 * <p> <b>Objective:</b>Objective: Verify to which demi decile graph should default</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8617.aspx </p>
 * <p> <b>Module:</b> Audience Builder</p>
 * @author Amrutha Nair
 * @since 06-May-2016
 *
 */
public class AudienceDefinition8617 extends BaseClass {
		
	@Test
	public void verifyDefaultDemiDecileGraph(){
		String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8617-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8617-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8617_ProjectName", "fail");
			}
			...
			if(AD.isVisible("demidecilegraph", "")){
				...
				throw new IDIOMException(strMsg+" .###8617-DemiDecileGraphNotPresnt");
			}
			
			//Step 8:Now verify to which demi decile graph should default
			
			int[] sliderData=AD.returnSliderData();
			
			if(sliderData[0]==0 && sliderData[1]==100){
				strMsg="By default, the demidecile selected values are '0' and '100' ";
				CustomReporter.log(strMsg);
			}
			else{
				strMsg="By default, the demidecile selected values are not '0' and '100'. By default , '"+sliderData[0]+"'is selected at left side and '"+sliderData[1]+"' is selected at right side";
				throw new IDIOMException(strMsg+" .###8617-DemiDecileGraphNotPresnt");
			}
			...
			rm.captureScreenShot("8617", "fail");
		}finally{
			try{
