...
/** 
 * <p> <b>Test Case Name:</b>Audience 2.0 : Super Admin User Journery</p>
 * <p> <b>Objective:</b> Verify user journey for Super Admin User </p>
 * <p> <b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8660.aspx</p>
 * <p> <b>Module:</b> Audience Builder</p>
 * @author Amrutha Nair
 * @since 12 May 2016
 *
 */
public class AudienceDefinition8660 extends BaseClass {

	@Test
	public void verifySuperAdminUserJourney(){
	String strMsg = null;
		...
		try{
			...
			if(AD.verifySubcategoriesForSelectedSuccessMetrics(strSubCategories, expectedCategories)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_SubCategoriesInSuccessMetrics", "fail");
			}
			...
			if(AD.getCountForSuccessMetricsOrAudienceDefinition()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_NumberofslectedCards", "fail");
			}
			...
			if(AD.isVisible("removealllink")){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_RemoveAll", "fail");
			}
			...
			if(AD.getCountForSubCategoriesHavingCorssIcon()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_CrossSignissue", "fail");
			}
			...
			if(AD.getTotalMetricCards()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_SelectedMetricCard", "fail");
			}
			...
			if (strActualVal.contains(strmetricsToBeRemoved.trim())){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_DidNotRemoveSuccessMetric", "fail");
			}
			else{
			...
			if(AD.verifySuccessMetricsSearchfunctionality(strSearchValue)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_SearchFunctionality", "fail");
			}
			...
			if(AD.isVisible("defaultgroup", "")){
				...
				CustomReporter.errorLog(strMsg);
				BaseClass.testCaseStatus=false;
				rm.captureScreenShot("8660NoDefaultGroup", "fail");
			}
			...
			if(AD.getCountForSuccessMetricsOrAudienceDefinition()==1){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_Numberofqueries", "fail");
			}
			...
			if(strQueryLogic.toLowerCase().contentEquals(defaultQueryOrSubGroupLogic.toLowerCase())){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_QueryLogicdefault", "fail");
			...
			if(strQueryLogic.toLowerCase().contentEquals(strLogic)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_QueryLogicChange", "fail");
			}
			...
			if(AD.getQueryWithLogic("1.1").contentEquals(strQuery)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_QueryNotgotdeleted", "fail");
			}
			...
			if(AD.getGroupCount()==(grpCount+1)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_AddGroupissue", "fail");
			}
			...
			if(AD.getSubGroupCount("1")==(intsubgrp+1)){
				...
				CustomReporter.errorLog(strMsg);
			    BaseClass.testCaseStatus=false;
			    rm.captureScreenShot("8660_AddsubGroupissue", "fail");
			}
			...
			if(!rm.webElementSync(AP.homeOwnerGraph, "visibility"))
				throw new IDIOMException("Failed to land on profile page.###8660_NotAbleToNavigateToProfilePage");
			...
			if(strMsgAndFileName.length==1){
				rm.captureScreenShot("8660-Exception", "fail");
			}else{
				rm.captureScreenShot("8660-"+strMsgAndFileName[1], "fail");	
			}			
