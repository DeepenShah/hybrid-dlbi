...
/** 
 * <p> <b>Test Case Name:</b>*1976: Verify blank group name is Not allowed</p>
 * <p> <b>Objective:</b>Objective: Verify that blank group name is not allowed   </p>
 * <p> <b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8702.aspx	 </p>
 * <p> <b>Module:</b> Audience Builder</p>
 * @author Amrutha Nair
 * @since 11 May 2016
 *
 */
public class AudienceDefinition8702 extends BaseClass {

	@Test
	public void verifyBlankGroupNameSave(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8702-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8702-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8702-ProjectName", "fail");
				
			}
			
			
			//Step 7:	Define few success metrics or do not define
			...
			//Step 9:Create a group with attributes
			...
			strMsg="The user has added attributes  in the default group";
			CustomReporter.log(strMsg);
			
			//Step 10:Click on group name
			AD.clickToEditGroupName("1");
		
			strMsg="The user has clicked on the name of the default group";
			CustomReporter.log(strMsg);
			
			//Step 11:Remove the group name
			AD.fillGroupName("");
			
			if(AD.isVisible("editgroupdisbaledbutton", "")){
				strMsg="The 'Ok' button for saving the group name is getting disabled on inputting blank group name";
				CustomReporter.log(strMsg);
			}
			
			
			else{
				strMsg="The 'Ok' button for saving the group name is NOT getting disabled on inputting blank group name";
				throw new IDIOMException(strMsg+".###8702-BlankGroupName");
			}
			...
			if(strMsgAndFileName.length==1){
				rm.captureScreenShot("8702-Exception", "fail");
			}else{
				rm.captureScreenShot("8702-"+strMsgAndFileName[1], "fail");	
			}			
