...
/** 
 * <p> <b>Test Case Name:</b>**Webnet_Verify data getting Updated On AddingDifferentAttributes</p>
 * <p> <b>Objective:</b> Verify that webnetdata is getting updated on adding different attributes</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/9103.aspx</p>
 * <p> <b>Module:</b> webnet</p>
 * @author Amrutha Nair
 * @since 31-May-2016
 *
 */
public class Webnet9103 extends BaseClass {
	...
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###9103-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###9103-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("9103_projectname", "fail");
			}
			...
			//Step 10:	Note the data in webnet for default transition window
			...
			//Step 11:Navigate to audience definition page
			
			PH.megaMenuLinksClick(property.getProperty("audienceDefinition"));
			rm.webElementSync(AD.allMetricsLabel,"verifytext", "ALL ATTRIBUTES");
			strMsg="The user has landed on audience definition page";
			CustomReporter.log(strMsg);
			
		
			//Step 12:Add a few attributes
			Thread.sleep(2000);
			...
			strMsg="The user has added an attribute in default group ";
			CustomReporter.log(strMsg);

			
			//Step 13:From Mega Menu click on Webnet
			...
			//step 14:	Check the data in default transition window
			
			chartData1=webnet.captureDataInWindow();
			
			if(webnet.CompareCharts(chartData1,chartData)){
				
				strMsg="The data is not getting updated on adding different attributes for the project'"+strProjectName;
				System.out.println(strMsg);
				throw new IDIOMException(strMsg+"###9103_ChartDataNotGettingUpdated");
			}
			else{
				strMsg="The data is  getting updated on adding different attributes for the project'"+strProjectName;
				CustomReporter.log(strMsg);
			}
				...
				rm.captureScreenShot("9103", "fail");
			}finally{
				try{
