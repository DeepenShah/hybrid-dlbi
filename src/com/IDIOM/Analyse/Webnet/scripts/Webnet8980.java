...
/** 
 * <p> <b>Test Case Name:</b>*Web Net: Values in webnet window should updated on changing values into Transition Window drop down</p>
 * <p> <b>Objective:</b> Verify that web net values should be changed according to the changing values into Transition Window drop down</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8980.aspx</p>
 * <p> <b>Module:</b> webnet</p>
 * @author Amrutha Nair
 * @since 31-May-2016
 *
 */
public class Webnet8980 extends BaseClass {
		
	@Test
	public void verifyTransitionWindowItemChange(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8980-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8980-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8980_projectname", "fail");
			}
			...
			if(webnet.getSelectedTransitionWindowItem(1).trim().toLowerCase().contentEquals(transitionWindow.toLowerCase())){
				...
				CustomReporter.errorLog(strMsg);
				rm.captureScreenShot("8980_DefaultDataTranistionWindow", "fail");
				BaseClass.testCaseStatus=false;
			}
			...
			if(transitionWindowExpected.length==transitionActual.size()){
				...
				if(status){
					...
					CustomReporter.errorLog(strMsg);
					rm.captureScreenShot("8980_TransitionWindowItemsIssue_", "fail");
					BaseClass.testCaseStatus=false;
				...
				strMsg="The transition window doesn't contain expected items . The expected is ' "+transitionWindowItems+"' But actual is '"+transitionActual+"'";
				CustomReporter.errorLog(strMsg);
				rm.captureScreenShot("8980_TransitionWindowItemsIssue", "fail");
				BaseClass.testCaseStatus=false;
			}
		
			
			//Step 12:Change value as "One Day" in transition window drop down
			//Step 13:Change value as "One Hour" in transition window drop down
			//Step 14:Change value as "One Week" in transition window drop down
			//Step 15:Change value as "One Month" in transition window drop down
			for(int i=1;i<transitionWindowExpected.length;i++){
				webnet.selectTransitionWindowItem(transitionWindowExpected[i]);
				strMsg="The user has selected '"+transitionWindowExpected[i]+"' from dropdown ";
				CustomReporter.log(strMsg);
				rm.webElementSync("idiomspinningcomplete");
				Thread.sleep(2000);
				if(webnet.getSelectedTransitionWindowItem(i+1).trim().toLowerCase().contentEquals(transitionWindowExpected[i].toLowerCase())){
					strMsg="The selected value in transition window is '"+transitionWindowExpected[i]+"' as expected";
					CustomReporter.log(strMsg);
				}
				else{
					strMsg="The selected value in transition window is NOT '"+transitionWindowExpected[i]+"'. It is '"+webnet.getSelectedTransitionWindowItem(i+1)+"'";
					CustomReporter.errorLog(strMsg);
					rm.captureScreenShot("8980_DefaultDataTranistionWindow_"+i, "fail");
					BaseClass.testCaseStatus=false;
				}
				chartData1=webnet.captureDataInWindow();
				
				if(webnet.CompareCharts(chartData1,chartData)){
					strMsg="The data is not getting updated on selecting different transition window item '"+transitionWindowExpected[i]+"' from drop down";
					CustomReporter.errorLog(strMsg);
					rm.captureScreenShot("8980_TransitionWindowItemsDataIssue_"+i, "fail");
					BaseClass.testCaseStatus=false;
				}
				else{
					strMsg="The data is getting updated on selecting '"+transitionWindowExpected[i]+"' as expected";
					CustomReporter.log(strMsg);
				}
				...
				rm.captureScreenShot("8980", "fail");
			}finally{
				try{
