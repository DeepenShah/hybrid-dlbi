/********************************************************************
Test Case Name: *HVA_1.A.ii_Verify Bar Chart Vertical Scroll
Test Case ID:http://qa.digitas.com/SpiraTest/523/TestCase/7989.aspx
Objective:Verify that a vertical scroll bar's displayed and enabled when the amount of bar rows extend past the chart's viewable area.
Module: Analyse>HVA
...
public class HVA7989 extends BaseClass {
		
	@Test
	public void test_HVA7989() throws Exception{
	//****************Variables declaration and Initialization****************************
		...
		try{
			...
			if (!strActualVal.contains(strExpectedSuccessMetric1.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric1 + "' has not been added successfully. ###7989_didNotAddSuccessMetric");

			if (!strActualVal.contains(strExpectedSuccessMetric2.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric2 + "' has not been added successfully. ###7989_2_didNotAddSuccessMetric");
			
			if (!strActualVal.contains(strExpectedSuccessMetric3.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric3 + "' has not been added successfully. ###7989_3_didNotAddSuccessMetric");
			...
			if(!rm.webElementSync(ah.behaviour,"visibility"))
				throw new IDIOMException("Behavior dropdown is not available or there might be other issues on the page. ###7989_BehaviorsDropdownNotVisible");
			
			//Step 12 - Check when amount of rows in the chart extends past the viewable area
				if(ah.func_VerifyScroll()){
					CustomReporter.log("The scroll is proper in HVA page");
				}
				else{
					throw new IDIOMException("The scroll is absent for HVA page even after having more values. ###7989_ScrollAbsent");
				}
			}
				...
				rm.captureScreenShot("7989", "fail");
			}finally{
				try{
