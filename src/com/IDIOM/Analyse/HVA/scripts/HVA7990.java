...
/** 
 * <p> <b>Test Case Name:</b>**HVA_1.A_Verify Positive Values Bars</p>
 * <p> <b>Objective:</b> Verify the correct display of positive value bars in the chart.</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/7990.aspx</p>
 * <p> <b>Module:</b> HVA</p>
 * @author Amrutha Nair
 ...
 *
 */
public class HVA7990 extends BaseClass {
	
	@Test
	public void verifyPositiveValuesInHVAGraph() throws Exception{
		String strMsg = null;
		...
		try{
			...
			String BGColourBar=property.getProperty("backgroundcolorPositiveBars");
			String BarAligment=	property.getProperty("backgroundAlignmentPositiveBars");
			String LabelAligment=property.getProperty("backgroundAlignmentPositiveLabels");
			String ValuesAligment=property.getProperty("backgroundAlignmentPositveValues");
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###7990-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###7990-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("7990_projectname", "fail");
			}
			...
			for(int i=0;i<behaviours.size();i++){
				System.out.println(behaviours.get(i));
			...
			//Step 12:	Check bars color and on which side it extends when values > 0 	
			//Step 13:Check for the actions' name labels locations in the page when values > 0 
			//Step 14:Check for the actions' numerical value labels positions in the page, when vales > 0	
				
				if(hva.PositiveValuesNames.size()>0){
					strMsg="The positive bars are present for the behaviour  '"+behaviours.get(i)+"' ";
					CustomReporter.log(strMsg);
					if(hva.func_CheckBGColour("PositiveBarsColour", BGColourBar)){
						strMsg="The back ground colour for all positive bars are as expected for '"+behaviours.get(i)+"' ";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The back ground colour for all positive bars are not as expectedfor '"+behaviours.get(i)+"' ";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("7990_BGColour_"+i, "fail");
					}
					
					if(hva.verifyAlignment("PositiveBarsAlignment", BarAligment)){
						strMsg="The bar alignment  for all positive bars are as expected for '"+behaviours.get(i)+"'";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The bar alignment for all positive bars are not as expected '"+behaviours.get(i)+"'";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("7990_BarAlignment_"+i, "fail");
					}
					if(hva.verifyAlignment("PositiveLabelAlignment", LabelAligment)){
						strMsg="The label  alignment  for all positive bars are as expected '"+behaviours.get(i)+"'";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The label alignment for all positive bars are not as expected '"+behaviours.get(i)+"'";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("7990_labelAlignment_"+i, "fail");
					}
					
					
				
					if(hva.verifyAlignment("PositiveValuesAlignment", ValuesAligment)){
						strMsg="The values  alignment  for all positive bars are as expected '"+behaviours.get(i)+"'";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The values alignment for all positive bars are not as expected '"+behaviours.get(i)+"'";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("7990_valueAlignment_"+i, "fail");
					}
					
				}
				
				else{
					strMsg="The positive bars are  not present for the behaviour  '"+behaviours.get(i)+"' ";
					CustomReporter.log(strMsg);
				}
			...
			rm.captureScreenShot("7990", "fail");
		}finally{
			try{
