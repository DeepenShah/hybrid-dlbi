/********************************************************************
Test Case Name: *HVA_FullUX_Verify Bar Chart Background line 
Test Case ID:http://qa.digitas.com/SpiraTest/523/TestCase/7993.aspx
Objective:Verify that the bars in the chart are drawn against a background of vertical parallel "measuring" lines and that their lengths are proportional, relative to one another.
Module: Analyse>HVA
...
public class HVA7993 extends BaseClass {
	
	
	@Test
	public void test_HVA7993() throws Exception{
	...
	//****************Test step starts here************************************************
		...
		if(!rm.webElementSync(ah.behaviour,"visibility"))
			throw new IDIOMException("Behavior dropdown is not available or there might be other issues on the page. ###7993_BehaviorsDropdownNotVisible");
	...
	//Step 12:Check for the action bars length and numerical values
			Thread.sleep(3000);
			if(ah.func_VerifyChartBarWithValues()){
				CustomReporter.log("The bars' lengths are proportional based on their numerical values and the amount of background lines they cross.");
				
			}
			else{
				/*CustomReporter.errorLog("The bars' lengths are NOT proportional based on their numerical values and the amount of background lines they cross.");
				BaseClass.testCaseStatus=false;*/
				throw new IDIOMException("The bars' lengths are NOT proportional based on their numerical values and the amount of background lines they cross. ###7993_LengthsNotProposional");
			}
		}
		else
		{
			...
			rm.captureScreenShot("7993", "fail");
		}finally{
			try{
