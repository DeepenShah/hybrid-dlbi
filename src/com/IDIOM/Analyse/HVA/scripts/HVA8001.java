...
/** 
 * <p> <b>Test Case Name:</b>*HVA_2.A.ii & 2.A.iii_Verify zero behaviors filter</p>
 * <p> <b>Objective:</b> Verify the HVA bar chart generation when there aren't any selected behavior's from the Idiom's audience. .</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8001.aspx</p>
 * <p> <b>Module:</b> HVA</p>
 * @author Amrutha Nair
 ...
 *
 */
public class HVA8001 extends BaseClass {
		
	@Test
	public void verifyZeroBehaviourFilter() throws Exception{
		String strMsg = null;
		...
		if(clientListPage.func_ClientListPageElementExist("audiencetab"))
			throw new IDIOMException("Not able to verify new project window###8001-CreateProjectWindow");
		...
		if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
			throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8001-AudienceTabNotFound");
		...
		if(pdp.isVisible("projectname", strProjectName)){
			...
			rm.captureScreenShot("8001_projectname", "fail");
		}
		...
		if(AD.isVisible("successmetricsdefaultmessage")){
			...
			CustomReporter.errorLog(strMsg);
			rm.captureScreenShot("8001_NoSuccessMetricsMsg", "fail");
			BaseClass.testCaseStatus=false;
		}
		...
		//Step 10:Do not select any attribute and click on HVA link from mega menu
		
		PageHeader PH= new PageHeader(driver);
		PH.megaMenuLinksClick(property.getProperty("hva"));
		
		CustomReporter.log("The user has clicked on HVA link from megamenu");
		HVA_Page hva = new HVA_Page(driver);
		
		rm.webElementSync("idiomspinningcomplete");	
		//Step 11:	Verify the Behaviors Filters are present and click on one of them.
		...
		if(rm.webElementSync(hva.behaviour,"visibility")){
			...
			throw new IDIOMException(strMsg+"###8001_BehaviourFilterAbsent");
		}
		
		ArrayList<String> behaviour= hva.func_CaptureList("Behaviour");
		if(successMetrics.size()==behaviour.size()){
			strMsg="All the expected bhaviours are presnet in behaviour drop down";
			CustomReporter.log(strMsg);
		}
		else{
			strMsg="All the expected bhaviours are not present in behaviour drop down";
			CustomReporter.errorLog(strMsg);
			rm.captureScreenShot("8001_BhaviourDropDownIssue", "fail");
			BaseClass.testCaseStatus=false;
		}
		
		Hashtable<String, String> ChartData1=hva.func_CaptureChartValues(behaviour.get(0));
		
		Hashtable<String, String> ChartData2=null;
		if (behaviour.size()>1){
			for(int i=1;i<behaviour.size();i++){
				
				//Select first behaviour from behaviour drop down and cature the values in a hash table
				ChartData2=hva.func_CaptureChartValues(behaviour.get(i));
				CustomReporter.log("The user has selected '"+behaviour.get(i)+"'from behaviour drop down and captured values in hash table");
			
			    //Compare the chart data for two items
				if(hva.func_CompareCharts(ChartData2, ChartData1)){
					CustomReporter.log("The bar chart is getting updated on selecting different filters");
				}
				else{
					CustomReporter.errorLog("The bar chart is Not getting updated on selecting differnt charts");
					BaseClass.testCaseStatus=false;
				}
			}
		}
			...
			CustomReporter.log("There is only one behaviour filter appears");
		}
			...
			rm.captureScreenShot("8001", "fail");
		}finally{
			try{
