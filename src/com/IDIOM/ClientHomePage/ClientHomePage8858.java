...
/********************************************************************
<p>	<b>Test Case Name:</b> Manage Projects 2.b.vii : Cancel renaming audience. </p>
<p>	<b>Objective:</b> To verify 'Cancel' rename audience. </p>
<p>	<b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8858.aspx </p>
<p>	<b>Module:</b> Client Home Page </p>
@author: Shailesh Kava
@since: 13-June-2016
**********************************************************************/
public class ClientHomePage8858 extends BaseClass {
	...
	public void renameAudience(){
		...
		String strMsg = null;
		String strProjectName = "8858_"+rm.getCurrentDateTime();
		String strAudName = "8858Aud_"+rm.getCurrentDateTime();
		...
		try{
			...
			//Launch the project
			clientListPage.performActionOnAudience(strAudName, "cancelrename");
			...
			for(int i=0; i<getAudienceListFromHeaderDropDown.size(); i++){
				if(getAudienceListFromHeaderDropDown.get(i).trim().equalsIgnoreCase(renameAudienceName)){
					rm.captureScreenShot("8858_failedToCancelAudience", "fail");
					System.out.println("Audience name is updated on cancel action, '8858_failedToModifiedAudience.png'");
					CustomReporter.errorLog("Audience name is updated on cancel action, '8858_failedToModifiedAudience.png'");
				...
				if(i==getAudienceListFromHeaderDropDown.size()-1){
					if(!getAudienceListFromHeaderDropDown.get(i).trim().equalsIgnoreCase(renameAudienceName)){
						System.out.println("Audience name is not changed on cancel action");
						CustomReporter.log("Audience name is not changed on cancel action");
					}
				}
			}
			...
			rm.captureScreenShot("8858", "fail");
		}finally{
			try{
