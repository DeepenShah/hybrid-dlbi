...
/********************************************************************
<p><b>Objective</b> Global Header - Verify the presence of Audience and its click in Header.</p>
<p><b>Test Case Name</b> Global Header - Changing Audiences - Verify the presence of Audience names and its click in Header.</p>
<p><b>Test Case ID</b> http://qa.digitas.com/SpiraTest/523/TestCase/8839.aspx</p>
<p><b>Module</b> Global Header</p>
@author Shailesh Kava
@since 21 June 2016
**********************************************************************/
public class GlobalHeader8839 extends BaseClass {
	boolean bProjectCreate = false;
	String strDetails;
	Login_Page lp;
	
	@Test
	public void verifyListAudienceInAllPages() throws Exception{
	//****************Variables declaration and Initialization****************************
		String pathingLink = property.getProperty("pathing");	
		String strMsg = null;
		String strProjectName="8839_"+rm.getCurrentDateTime();
		String strAudName = "8839_"+rm.getCurrentDateTime();
	...
	try{
		...
		ArrayList<String> arrListAudienceInHeader;
		...
		for(String linkName:arrLinkNames){
			...
			if(linkName.equalsIgnoreCase(property.getProperty("projectDescription"))){
				...
				if(!selectedAudName.equalsIgnoreCase(strDefaultAud)){
					rm.captureScreenShot("8839_changedAudIsNotSelectedInClintListPage", "Fail");
			...
			if(linkName.equalsIgnoreCase(property.getProperty("successMetrics"))){
				...
				if(!audienceBuilder.isVisible("nosuccessmetrictext", "")){
					rm.captureScreenShot("8839_failedToOpen"+linkName+"Page", "fail");
					...
					//getting list of audience
					pageHeader.audienceDropDownLink.click();
					Thread.sleep(1000);
					arrListAudienceInHeader = pageHeader.returnAudiencesInDropDown();
					System.out.println("Audiences "+arrListAudienceInHeader);
					
					if(!arrListAudienceInHeader.contains(strAudName.trim()) && !arrListAudienceInHeader.contains(strDefaultAud.trim()))
						throw new IDIOMException("Audience names are not matched in page " +
								linkName + " it should be " + strAudName +" and "+strDefaultAud+"8839_###audienceAreNotMatchedIn"+linkName);
					
					CustomReporter.log("Audience names are matched in "+linkName);
					pageHeader.audienceDropDownLink.click();
				}
			...
			if(linkName.equalsIgnoreCase(property.getProperty("audienceDefinition"))){
				...
				if(!audienceBuilder.isVisible("audiencedefinitiontab")){
					...
					if(!arrListAudienceInHeader.contains(strAudName) && !arrListAudienceInHeader.contains(strDefaultAud))
						throw new IDIOMException("Audience names are not matched in page " +
								linkName + " it should be " + strAudName +" and "+strDefaultAud+"8839_###audienceAreNotMatchedIn"+linkName);
		...
		rm.captureScreenShot("8839", "fail");
	}finally{
		try{
