...
/**
 * <p> <b>Test Case Name:</b> *Common Elements - 2.b.i Population - Verify the Population Panel rebranding of Future Behaviors to Success metrics </p>
 * <p> <b>Objective:</b> Verify the Population Panel re branding of Future Behaviors to Success metircs </p>
 * <p> <b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8579.aspx </p>
...
public class SuccessMetrics8579 extends BaseClass{

@Test
	
	public void	SuccessMetrics8579(){
		String strMsg = null;
		...
		try{
			...
			String strsuccessMetricsTitleInLHS = property.getProperty("successMetricsTitleInLHS");
			...
			if (!strActualVal.contains(strExpectedSuccessMetric1.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric1 + "' has not been added successfully. ###8579_didNotAddSuccessMetric");

			if (!strActualVal.contains(strExpectedSuccessMetric2.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric2 + "' has not been added successfully. ###8579_2_didNotAddSuccessMetric");
			
			if (!strActualVal.contains(strExpectedSuccessMetric3.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric3 + "' has not been added successfully. ###8579_3_didNotAddSuccessMetric");

			CustomReporter.log("Selected Success Metrics appear in centre area.");

			//Step 9 - Verify the Population Panel re branding of Future Behaviors to Success metrics
			
			CustomReporter.log("Verify the Population Panel re-branding of Future Behaviors to Success Metrics");
			
			if (!(strsuccessMetricsTitleInLHS.trim().toLowerCase().equalsIgnoreCase(audienceBuilder.successMetricsTextTitle.getText().trim().toLowerCase())))
				throw new IDIOMException("Success Metrics Title is not noticed in LHS. ###8579_SuccessMetricsTitleNotNoticed");
			
			CustomReporter.log("Future Behaviors are now rebranded to Success Metrics.");
