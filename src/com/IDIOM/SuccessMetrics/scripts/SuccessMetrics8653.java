...
/********************************************************************
<p>	<b>Test Case Name:<b> Success Metrics - Verify count at the RHS at the bottom when filter cards are removed.
<p>	<b>Objective:<b> Success Metrics - Verify count at the RHS at the bottom when filter cards are removed.
<p>	<b>Test Case ID:<b> http://qa.digitas.com/SpiraTest/523/TestCase/8653.aspx
<p>	<b>Module:<b> Success Metrics
@author:<b> Abhishek Deshpande
@since:<b> 10-May-2016
**********************************************************************/

public class SuccessMetrics8653 extends BaseClass {
	
	@Test
	public void countSuccessMetric(){
		String strMsg = null;
		ClientList_Page clientListPage=null;
		String strProjectName="Test Case 8653";
		...
		try{
			...
			//Step5: Add few success metrics cards
			...
			if(!audienceBuilder.isVisible("successmetricstexttitle", ""))
				throw new IDIOMException("Success metrics page is not visible###testcase8653-pageIsNotVisible");
			...
			rm.captureScreenShot("Test8653MetricCardHasBeenAdded", "pass");
			
			
			//Step6: Remove few success metrics and verify the count on the RHS after few filters were removed.							
			...
			int count = allSuccessMetrics.size();
			for(int i=0;i<allSuccessMetrics.size();i++){	
				
				if(!(successMetricPage.getCountForSuccessMetricsOrAudienceDefinition()==count)) //This will verify success metrics count after removing each filter
					throw new IDIOMException("Failed to verify the success metrics count ###testcase8653-successmetricscount");
				...
				System.out.println(allSuccessMetrics.size());
				Thread.sleep(2000);
			}
			...
			//Verify no success metrics message
			 if(!(successMetricPage.getNoSuccessMetricsSelectedMessage().equals(message)))
				 throw new IDIOMException("Failed to verify no success metric message ###testcase8653-NoSuccessMetricmessage");			 
