...
/**
 * <p> <b>Test Case Name:</b> *Common Elements - 2.b.iii.1 Population - Verify after success metrics are added, the message that will be hidden </p>
 * <p> <b>Objective:</b> Verify after success metrics are added, the message that will be hidden </p>
 * <p> <b>Test Case ID:</b> http://qa.digitas.com/SpiraTest/523/TestCase/8586.aspx </p>
 * <p> <b>Module:</b> Audience Builder - Success Metrics</p>
 * @author Rohan Macwan
 * @since 12 May 2016
 *
 */
public class SuccessMetrics8586 extends BaseClass{

	@Test
	
	public void	SuccessMetrics8586(){
		String strMsg = null;
		...
		try{
			...
			if (!((audienceBuilder.isVisible("selectedactivetab", "") && (audienceBuilder.SelectedActiveTab.getText().trim().toLowerCase().equalsIgnoreCase("Success Metrics")))))
				throw new IDIOMException("Success Metrics Tab seems to be not available or there may be some issues on the page. ###8586_SuccessMetricsTabNotAvailable");
			...
			if (!strActualVal.contains(strExpectedSuccessMetric1.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric1 + "' has not been added successfully. ###8586_didNotAddSuccessMetric");

			if (!strActualVal.contains(strExpectedSuccessMetric2.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric2 + "' has not been added successfully. ###8586_2_didNotAddSuccessMetric");
			
			if (!strActualVal.contains(strExpectedSuccessMetric3.trim()))
				throw new IDIOMException("Success Metric '" + strExpectedSuccessMetric3 + "' has not been added successfully. ###8586_3_didNotAddSuccessMetric");

			CustomReporter.log("Selected Success Metrics appear in centre area.");
						
			CustomReporter.log("Check Default Message in LHS whether it is present");
			
			if ((audienceBuilder.isVisible("successmetricsdefaultmessage"))) 
				throw new IDIOMException("Default Message in LHS still appear. ###8586_DefaultMessageInLHSStillAppear");
			
			CustomReporter.log("Default Message in LHS has correctly disappeared");
		
			String strCenterAreaMsg="";
			strCenterAreaMsg = audienceBuilder.getSuccessMetricsMsgOfCenterArea();
			
			CustomReporter.log("Check Default Message in Center Area whether it is present");
			
			if ((audienceBuilder.isVisible("nosuccessmetrictext"))) 
				throw new IDIOMException("Default Message in Center area still appear. ###8586_DefaultMessageInCenterStillAppear");
			
			CustomReporter.log("Default Message in Center area has correctly disappeared");
			...
			rm.captureScreenShot("8586", "fail");
		}finally{
			try{
