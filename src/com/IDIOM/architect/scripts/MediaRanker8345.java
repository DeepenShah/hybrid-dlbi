...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.b.ix_Verify_FutureDates_Are_Not_Allowed</p>
 * <p> <b>Objective:</b>Verify whether future dates are not allowed </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8345.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 24-May-2016
 *
 */
public class MediaRanker8345 extends BaseClass {
		
	@Test
	public void verifyFutureDatesAreNotAllowed(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8345-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8345-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8345_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 12:	Verify whether future dates are not allowed

					
					if(!AM.isFuturedateAllowed()){
						strMsg="The future dated are not allowed to select  for "+strMediaRanker[k]+"' as expected ";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The future dated are  allowed to select  for "+strMediaRanker[k]+"' ";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8345_FutureDatesAllowed_"+k, "fail");
					}
			...
			rm.captureScreenShot("8345", "fail");
		}finally{
			try{
