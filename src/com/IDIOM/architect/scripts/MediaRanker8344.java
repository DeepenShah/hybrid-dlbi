...
/** 
 * <p> <b>Test Case Name:</b>Media Ranker Overview_1.b.i.2Display and Timeframe_.b.viii.1.a_Verify_recent 30 days with data are saved</p>
 * <p> <b>Objective:</b> Verify_recent 30 days with data are saved </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8344.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 23-May-2016
 *
 */
public class MediaRanker8344 extends BaseClass {
		
	@Test
	public void verifyrecent30DaysWithDataSaved(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8344-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8344-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8344_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 12:Verify recent 30 days with data are selected in widget
					...
					if(AM.selectedDatesList.size()<=30){
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8344_NumberOfDatsSelected_"+k, "fail");
					}
			...
			rm.captureScreenShot("8344", "fail");
		}finally{
			try{
