...
/** 
 * <p> <b>Test Case Name:</b>Media Ranker Overview_1.b.i.2Display and Timeframe_.b.v_Verify_Cancel_Button_Click</p>
 * <p> <b>Objective:</b>   Verify Cancel Button Click </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8338.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 20-May-2016
 *
 */
public class MediaRanker8338 extends BaseClass {
		
	@Test
	public void verifyCancelButtonClick(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8338-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8338-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8338_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					CustomReporter.log(strMsg);
					Thread.sleep(3000);
					strBubbles =AM.func_GetValue("totalcirclecountfromheader") ;
					strMsg="The bubbles present before selecting another date range is "+strBubbles;
					System.out.println(strMsg);
					CustomReporter.log(strMsg);
					
					intNumberOfDates =AM.selectedDatesList.size();
					strMsg="The tota1 selected dates before performing any action on date picker for '"+strMediaRanker[k]+"' is :"+intNumberOfDates;
					CustomReporter.log(strMsg);
					
					
					AM.func_ClickOnElement("calenderclearbtn");
					
					strMsg="The user clicked on clear button on date picker for '"+strMediaRanker[k]+"' ";
					CustomReporter.log(strMsg);
					...
					//Step 12:	Perform different operation on Date Widget and click on Cancel button
					if(AM.func_selectDate(1, "")){
						strMsg="The user selected a different daterange  for '"+strMediaRanker[k]+"' succcesssfully ";
						CustomReporter.log(strMsg);
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8338_selectdate_"+k, "fail");
					}
					
					
					AM.func_ClickOnElement("calendercancelbtn");
					strMsg="The user clicked on cancel button with selected a different daterange  for '"+strMediaRanker[k]+"' succcesssfully ";
					CustomReporter.log(strMsg);
					
					Thread.sleep(10000);
					if(strBubbles.contentEquals(AM.func_GetValue("totalcirclecountfromheader"))){
						strMsg="The data is not changing for  '"+strMediaRanker[k]+"'  on clicking on cancel after changing date ";						
						CustomReporter.log(strMsg);
						
					}
					else{
						strMsg="The data is  changing for  '"+strMediaRanker[k]+"'  on clicking on cancel after changing date ";						
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8338_DataChangingOnCancelClick_"+k, "fail");
					}
					
					//Click on Time Frame Date Widget
					Thread.sleep(5000);
					
					AM.func_ClickOnElement("datepicker");
					
					Thread.sleep(3000);
					if(intNumberOfDates==AM.selectedDatesList.size()){
						strMsg ="The number of selected dates are same as before clicking on Cancel for '"+strMediaRanker[k]+"'";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg ="The number of selected dates are not same as before clicking on Cancel for '"+strMediaRanker[k]+"'";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8338_DatesResetNotWorking_"+k, "fail");
					}
			...
			rm.captureScreenShot("8338", "fail");
		}finally{
			try{
