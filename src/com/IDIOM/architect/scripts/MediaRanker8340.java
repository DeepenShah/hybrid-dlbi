...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.b.vii_Verify_TimeFrame_Getting_Updated_After_Date_Range_Selection</p>
 * <p> <b>Objective:</b> Verify TimeFrame is getting updated after date range selection</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8340.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 20-May-2016
 *
 */
public class MediaRanker8340 extends BaseClass {
		
	@Test
	public void verifyUpdateDatePicker(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8340-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8340-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8340_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					strMsg="The total selected dates before performing any action on date picker for '"+strMediaRanker[k]+"' is :"+AM.selectedDatesList.size();
					CustomReporter.log(strMsg);
					...
					//Step 12:	Verify TimeFrame is getting updated after date range selection
					if(AM.func_selectDate(1, "")){
						...
						strMsg="The user not able to  select  deselected dates  for '"+strMediaRanker[k]+"' succcesssfully ";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8340_selectdate_"+k, "fail");
					}

					strMsg="The total selected dates after selecting dated  on date picker for '"+strMediaRanker[k]+"' is :"+AM.selectedDatesList.size();
					CustomReporter.log(strMsg);
					
					if(AM.selectedDatesList.size()==1){
						strMsg="The time frame widget is getting updated with number of dates selected for  '"+strMediaRanker[k]+"' succcesssfully.  ";
						CustomReporter.log(strMsg);
					}
					else {
						strMsg="The time frame widget is not  getting updated with number of dates selected for  '"+strMediaRanker[k]+"' succcesssfully.  ";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8340_selectdateWidgetNOTGettingUpdated_"+k, "fail");
					}
			...
			rm.captureScreenShot("8340", "fail");
		}finally{
			try{
