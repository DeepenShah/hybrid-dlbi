...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.5_Removing Bubbles_.a_Verify_Double_Click_Of_Bubble</p>
 * <p> <b>Objective:</b>:Objective: Verify double click of bubble</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8350.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 24-May-2016
 *
 */
public class MediaRanker8350 extends BaseClass {
		
	@Test
	public void verifyDoubleClickOnBubble(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8350-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8350-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8350_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					if(!(AM.verifyBubblePresent(bubbleName))){
						strMsg="Thedouble clicked  bubble with name '"+bubbleName+"' for '"+strMediaRanker[k]+"' got removed from scatterplot";
						CustomReporter.log(strMsg);
					}
					else{
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8350_DoubleClik_"+k, "false");
					}
			...
			rm.captureScreenShot("8350", "fail");
		}finally{
			try{
