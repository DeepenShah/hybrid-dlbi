...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.b.ii_Check_When_Non_Consecutive_Dates_Are_Selected</p>
 * <p> <b>Objective:</b>  Verify when non consecutive dates are selected </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8334.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 19-May-2016
 *
 */
public class MediaRanker8334 extends BaseClass {
		
	@Test
	public void verifyNonConsecutiveDatesSelection(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8334-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8334-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8334_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 12:Verify when non consecutive dates are selected

					
					if(AM.func_selectDateWRTPosition("1,3,5,7"))
					{
						strMsg="The user is able to select nonconsecutive dates  for '"+strMediaRanker[k]+"' ";
						CustomReporter.log(strMsg);
					}
					
					else{
						strMsg="The user not able to  select nonconsecutive dates  for  '"+strMediaRanker[k]+"' succcesssfully ";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8334_selectNonConsecutiveDates_"+k, "fail");
					}
			...
			rm.captureScreenShot("8334", "fail");
		}finally{
			try{
