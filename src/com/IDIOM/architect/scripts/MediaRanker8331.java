...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.a.ii.1_Verify_Changing_Order_Of_Items_Under_Categoriess</p>
 * <p> <b>Objective:</b>   Verify changing order of Items under categories</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8331.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 18-May-2016
 *
 */
public class MediaRanker8331 extends BaseClass {
	...
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8331-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8331-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8331_projectname", "fail");
			}
			...
			for (int i=0;i<strMediaRanker.length;i++){
				...
				//Step 11:		Click on Display dropdown and verify changing order of Items under categories
				Thread.sleep(5000);
				...
				CustomReporter.log(strMsg);
				 ...
				 intDisplayDropdownItems=disPlayDropDown.size();
				 System.out.println(intDisplayDropdownItems);
				 ...
				 for(String tempDisplayItem : disPlayDropDown){
					    if(disPlayDropDownExpected.contains(tempDisplayItem.toLowerCase())){
							...
							CustomReporter.errorLog(strMsg);
							BaseClass.testCaseStatus=false;
							rm.captureScreenShot("8331_displayDropDown_"+i, "false");
					 ...
					 //Click on Category
					 AM.func_ClickCategoryByNumber(0);
					 strMsg="The user has selected first category  for '"+strMediaRanker[i]+"'";
					...
					AM.func_VerifyVisibilityOfElement("datepicker", "");
					 ...
					 disPlayDropDown= AM.getDisplayDropDownItems();
					 if(disPlayDropDown.size() ==intDisplayDropdownItems-1){
						 strMsg="As expected only two display drop down items are coming for  '"+strMediaRanker[i]+"' since we have selected a category";
						 CustomReporter.log(strMsg);
						 
						 if(disPlayDropDown.contains(disPlayDropDownExpected.get(0))){
							 strMsg="The display dropdown contains '"+disPlayDropDown+"' still eventhough we have selected a category ";
							 CustomReporter.errorLog(strMsg);
							 BaseClass.testCaseStatus=false;
							 rm.captureScreenShot("8331_displayDropDownIssueonSelectingCategory_"+i, "false");
						       
						 }
						 else
						 {
							 strMsg="As expected The display dropdown doesn't contain '"+disPlayDropDownExpected.get(0)+"'  for  '"+strMediaRanker[i]+"' since we have selected a category";
							 CustomReporter.log(strMsg);
						 }
						 intDisplayDropdownItems= disPlayDropDown.size();
					 }
					 else{
						 strMsg="The display dropdown contains '"+disPlayDropDownExpected.get(0)+"' still  eventhough we have selected a category ";
						 CustomReporter.errorLog(strMsg);
						 BaseClass.testCaseStatus=false;
						 rm.captureScreenShot("8331_displayDropDownIssueonSelectingCategory_"+i, "false");
						 intDisplayDropdownItems=intDisplayDropdownItems-1;
					 }
					 ...
					 AM.func_ClickCategoryByNumber("",0);
					 rm.webElementSync("idiomspinningcomplete");
					 strMsg="The user has selected first sub category  for '"+strMediaRanker[i]+"'";
					 CustomReporter.log(strMsg);
					...
					disPlayDropDown=AM.getDisplayDropDownItems();
					 ...
					 if(disPlayDropDown.size() ==intDisplayDropdownItems-1){
						 strMsg="As expected only one display drop down items are coming for  '"+strMediaRanker[i]+"' since we have selected a sub category";
						 CustomReporter.log(strMsg);
						 
						 if(disPlayDropDown.contains(disPlayDropDownExpected.get(1))&&disPlayDropDown.contains(disPlayDropDownExpected.get(0))){
							 strMsg="The display dropdown contains '"+disPlayDropDown+"' still eventhough we have selected a sub category ";
							 CustomReporter.errorLog(strMsg);
							 BaseClass.testCaseStatus=false;
							 rm.captureScreenShot("8331_displayDropDownIssueonSelectingSubCategory_"+i, "false");
						       
						 }
						 else
						 {
							 strMsg="As expected The display dropdown doesn't contain '"+disPlayDropDownExpected.get(0)+"' and '"+disPlayDropDownExpected.get(1)+"'  for  '"+strMediaRanker[i]+"' since we have selected a sub category";
							 CustomReporter.log(strMsg);
						 }
						 intDisplayDropdownItems= disPlayDropDown.size();
					 }
					 else{
						 strMsg="The display dropdown contains '"+disPlayDropDownExpected.get(0)+"' and '"+disPlayDropDownExpected.get(1)+"' still  eventhough we have selected a subcategory ";
						 CustomReporter.errorLog(strMsg);
						 BaseClass.testCaseStatus=false;
						 rm.captureScreenShot("8331_displayDropDownIssueonSelectingsubCategory_"+i, "false");
						 intDisplayDropdownItems=intDisplayDropdownItems-1;
			...
			rm.captureScreenShot("8331", "fail");
		}finally{
			try{
