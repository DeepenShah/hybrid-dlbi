...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.4_Bubble Labels_.a_Verify_Data_On_Hovering_Of_Bubble</p>
 * <p> <b>Objective:</b>: Verify data on hovering of bubble </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8348.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 24-May-2016
 *
 */
public class MediaRanker8348 extends BaseClass {
		
	@Test
	public void verifyAxesValuesOnBubbleHover(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8348-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8348-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8348_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 11:	Verify data on hovering of bubble
				
					AM.func_ClickOnElement("reorderbtn");
					Thread.sleep(3000);
					if(AM.verifyAxisValuesWithBubbleOnHover()){
						strMsg="On hovering over a bubble, the data is properly getting displayed for '"+strMediaRanker[k]+"'. The X , Y and Z axis are as expected ";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="On hovering over a bubble, the data is not properly getting displayed for '"+strMediaRanker[k]+"'. The X , Y and Z axis are not as expected ";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8348_DataPopulatingOnHover_"+k, "fail");
					}
			...
			rm.captureScreenShot("8348", "fail");
		}finally{
			try{
