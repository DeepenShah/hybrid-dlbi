...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.b.viii.1_Verify_The_Limit_Of_Days_For_Which_Data_Can_Be_Processed</p>
 * <p> <b>Objective:</b> Verify the limit of days for which data can be processed </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8343.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 23-May-2016
 *
 */
public class MediaRanker8343 extends BaseClass {
		
	@Test
	public void verifyDateLimitForProcessing(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8343-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8343-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8343_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 12:	Verify the limit of days for which data can be processed

					
					if(AM.selectedDatesList.size()<=30){
						strMsg="The date picker is selected with  '"+AM.selectedDatesList.size()+"' dates for "+strMediaRanker[k]+"' ";
						CustomReporter.log(strMsg);
						
						AM.func_deselectSelectedDate(0, "all");
						strMsg="The user has deselected all the selected dates from the date picker";
						
						AM.func_selectDate(0, "all");
						strMsg="The user has selected all the deselected dates from the date picker succeefully";
					}
					else{
						strMsg="The date picker is selected with  '"+AM.selectedDatesList.size()+"' dates for "+strMediaRanker[k]+"' which is more than expetced limit";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8343_NumberOfDatsSelected_"+k, "fail");
					}
			...
			rm.captureScreenShot("8343", "fail");
		}finally{
			try{
