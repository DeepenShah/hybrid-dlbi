...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.b.iv_Verify_Apply_Button_Click</p>
 * <p> <b>Objective:</b>   Verify Apply Button click </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8337.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 19-May-2016
 *
 */
public class MediaRanker8337 extends BaseClass {
		
	@Test
	public void verifyApplyButtonClick(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8337-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8337-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8337_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					if(AM.func_deselectSelectedDate(0, "all"))
					{
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8337_deselectdate_"+k, "fail");
					}
					Thread.sleep(3000);
					
					//Step 12:Select different date range and click on 'apply'
					if(AM.func_selectDate(1, "")){
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8337_selectdate_"+k, "fail");
					}
					
					
					AM.func_ClickOnElement("calenderapply");
					strMsg="The user clicked on apply button with selected a different daterange  for '"+strMediaRanker[k]+"' succcesssfully ";
					CustomReporter.log(strMsg);
					
					Thread.sleep(10000);
					if(strBubbles.contentEquals(AM.func_GetValue("totalcirclecountfromheader"))){
						strMsg="The number of bubbles are not changing for  '"+strMediaRanker[k]+"'  on providing different date range  ";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The data is  changing for  '"+strMediaRanker[k]+"'  on providing different date range succcesssfully ";
						CustomReporter.log(strMsg);
			...
			rm.captureScreenShot("8337", "fail");
		}finally{
			try{
