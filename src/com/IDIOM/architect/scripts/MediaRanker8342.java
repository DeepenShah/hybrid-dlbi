...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.2Display and Timeframe_.b.viii_Verify_Default_Date_Range</p>
 * <p> <b>Objective:</b>Verify default date range</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8342.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 23-May-2016
 *
 */
public class MediaRanker8342 extends BaseClass {
	...
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8342-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8342-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8342_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 12:Verify default date range

					
					if(AM.selectedDatesList.size()<=30){
						strMsg="The date picker is selected with  '"+AM.selectedDatesList.size()+"' dates for "+strMediaRanker[k]+"' by default ";
						CustomReporter.log(strMsg);
					}
					else{
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8342_NumberOfDatsSelected_"+k, "fail");
					}
			...
			rm.captureScreenShot("8342", "fail");
		}finally{
			try{
