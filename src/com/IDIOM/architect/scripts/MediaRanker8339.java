...
/** 
 * <p> <b>Test Case Name:</b>Media Ranker Overview_1.b.i.2.bDisplay and Timeframe_.vi_Verify_Clicking_Outside_When_Date_Widget_Is_Open</p>
 * <p> <b>Objective:</b>Verify clicking Outside are when date widget is open</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8339.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 20-May-2016
 *
 */
public class MediaRanker8339 extends BaseClass {
	...
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8339-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8339-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8339_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 12:	Perform/Do not perform different operations and click on outside Widget area.
					if(AM.func_selectDate(1, "")){
						...
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8339_selectdate_"+k, "fail");
					}
					
					
					//click outside date widget
					//Click on Time Frame Date Widget
					Thread.sleep(5000);
					
					AM.func_ClickOnElement("weightedranker");
					strMsg="The user clicked outside date widget  for '"+strMediaRanker[k]+"'  ";
					CustomReporter.log(strMsg);
					
					Thread.sleep(3000);
					if(!AM.func_VerifyVisibilityOfElement("datepickeropenedwidget")){
						strMsg ="The date picker widget is getting closed on clicking outside the datepicker window  for  '"+strMediaRanker[k]+"'";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg ="The date picker widget is not getting closed on clicking outside the datepicker window  for  '"+strMediaRanker[k]+"'";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8339_DatePickerNotGettingClosed_"+k, "fail");
					}
			...
			rm.captureScreenShot("8339", "fail");
		}finally{
			try{
