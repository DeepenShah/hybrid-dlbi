...
/** 
 * <p> <b>Test Case Name:</b>**Media Ranker Overview_1.b.i.4_Bubble Labels_.b_Verify_Effect_On_Hovering_Of_Bubble</p>
 * <p> <b>Objective:</b>: Verify effect on hovering of bubble </p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8349.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 24-May-2016
 *
 */
public class MediaRanker8349 extends BaseClass {
		
	@Test
	public void verifyBubbleDesignOnHover(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8349-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8349-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8349_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 11:	Verify effect on hovering of bubble
				
					Thread.sleep(5000);
					if(AM.verifyBubblesDesignOnHover()){
						strMsg="On hovering over a bubble, the bubbles are getting highlighted as expected for  '"+strMediaRanker[k]+"'";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="On hovering over a bubble, the bubbles are NOT getting highlighted as expected for  '"+strMediaRanker[k]+"'";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8349_BubblesOnHover_"+k, "fail");
					}
			...
			rm.captureScreenShot("8349", "fail");
		}finally{
			try{
