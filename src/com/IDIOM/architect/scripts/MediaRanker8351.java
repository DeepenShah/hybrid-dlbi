...
/** 
 * <p> <b>Test Case Name:</b>*Media Ranker Overview_1.b.i.5_Removing Bubbles_.b_Verify_The_Reinstating_Of_Bubble</p>
 * <p> <b>Objective:</b> Verify the reinstating of bubble</p>
 * <p> <b>Test Case ID:</b>http://qa.digitas.com/SpiraTest/523/TestCase/8351.aspx</p>
 * <p> <b>Module:</b> Architect</p>
 * @author Amrutha Nair
 * @since 25-May-2016
 *
 */
public class MediaRanker8351 extends BaseClass {
		
	@Test
	public void verifyRemovedBubbleGetsAdded(){
	String strMsg = null;
		...
		try{
			...
			if(clientListPage.func_ClientListPageElementExist("audiencetab"))
				throw new IDIOMException("Not able to verify new project window###8351-CreateProjectWindow");
			...
			if(!(clientListPage.func_ClientListPageElementExist("audiencetab") && clientListPage.func_ClientListPageElementExist("audiencetablabel")))
				throw new IDIOMException("Fails to land on Audience tab. Problem might be in saving project###8351-AudienceTabNotFound");
			...
			if(pdp.isVisible("projectname", strProjectName)){
				...
				rm.captureScreenShot("8351_projectname", "fail");
			}
			...
			for (int k=0;k<strMediaRanker.length;k++){
				...
				if(AM.func_VerifyVisibilityOfElement("datepicker", "")){
					...
					//Step 11:	Verify double click of bubble
				
					Thread.sleep(3000);
					int bubbleID=AM.bubblesInScatterPlot.size()-1;
					
					String bubbleName=AM.performMouseOverBubbleAndGetToolTipName(bubbleID).trim().toLowerCase();
					
					AM.performDoubleClick(bubbleID);
					
					strMsg="The user has double clicked on bubble with name '"+bubbleName+"' for '"+strMediaRanker[k]+"'";
					CustomReporter.log(strMsg);
					
					Thread.sleep(3000);
					if(!(AM.verifyBubblePresent(bubbleName))){
						strMsg="The double clicked  bubble with name '"+bubbleName+"' for '"+strMediaRanker[k]+"' got removed from scatterplot";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="Thedouble clicked  bubble with name '"+bubbleName+"' for '"+strMediaRanker[k]+"' is not getting  removed from scatterplot";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8351_DoubleClik_"+k, "fail");
					}
				
					
					//Step 12:Go to Dataset builder and add that bubble again
					AM.toggleDataSetElement(bubbleName);
					strMsg="The user has selected the same bubble  '"+bubbleName+"' for '"+strMediaRanker[k]+"' from data selet builder";
					CustomReporter.log(strMsg);
					
					//Step 13:Now come back to Scatter Plot and check
					Thread.sleep(5000);
					if(AM.verifyBubblePresent(bubbleName)){
						strMsg="The bubble with  name '"+bubbleName+"' for '"+strMediaRanker[k]+"'is getting added in scatterplot";
						CustomReporter.log(strMsg);
					}
					else{
						strMsg="The bubble with name '"+bubbleName+"' for '"+strMediaRanker[k]+"' is not getting  added in scatterplot";
						CustomReporter.errorLog(strMsg);
						BaseClass.testCaseStatus=false;
						rm.captureScreenShot("8351_BubbleGettingAdded_"+k, "fail");
					}
			...
			rm.captureScreenShot("8351", "fail");
		}finally{
			try{
